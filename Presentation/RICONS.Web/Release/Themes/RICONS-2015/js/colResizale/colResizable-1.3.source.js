...
(function($){ 	
	...
	*/
	var syncCols = function(t,i,isOver){
	    ...
	    //table scrollTop
		var tbDanhSachTop = $('#boxContent .table_danhSach');
		tbDanhSachTop.find('th').eq(c.eq(0).index()).width(w + PX);
		tbDanhSachTop.find('th').eq(c2.eq(0).index()).width(w2 + PX);
		if (isOver) {
		    c.w = w; c2.w = w2;
		    tbDanhSachTop.find('th').eq(c.eq(0).index()).width(w);
		    tbDanhSachTop.find('th').eq(c2.eq(0).index()).width(w2);
		}
	};
