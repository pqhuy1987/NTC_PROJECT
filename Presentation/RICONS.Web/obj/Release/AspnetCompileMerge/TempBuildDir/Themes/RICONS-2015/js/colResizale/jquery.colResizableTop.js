...
(function($){ 	
	...
	var I = parseInt;
	var M = Math;
	var ie = browserInfo;//$.browser.msie; is not support in jQuery 1.9
	var S;
	...
	var init = function( tb, options){	
		...
		t.cs = 2;	//table cellspacing (not even jQuery is fully cross-browser)
		t.b  = 1;	//outer border width (again cross-browser isues)
	...
	var createGrips = function(t){	
		...
		th.each(function(i){						//iterate through the table column headers			
			...
			$('#tablee').find('tr:first-child td').eq(c.eq(0).index()).width(c.width());
	...
	var syncGrips = function (t){	
		...
		for(var i=0; i<t.ln; i++){	//for each column
			...
			t.g[i].css({			//height and position of the grip is updated according to the table layout
				...
				height: t.find('th').eq(0).outerHeight()				
			});			
		} 	
	};
	...
	*/
	var syncCols = function(t,i,isOver){
		...
		$('#tablee').find('tr:first-child td').eq(c.eq(0).index()).width(w + PX);
		$('#tablee').find('tr:first-child td').eq(c2.eq(0).index()).width(w2 + PX);
		if (isOver) {
		    c.w = w; c2.w = w2;
		    $('#tablee').find('tr:first-child td').eq(c.eq(0).index()).width(w + PX);
		    $('#tablee').find('tr:first-child td').eq(c2.eq(0).index()).width(w2 + PX);
	 ...
	 */
    $.fn.extend({  
        colResizableTop: function (options) {
            var defaults = {
				...
				marginLeft: '3px',				//in case the table contains any margins, colResizable needs to know the values used, e.g. "10%", "15em", "5px" ...
				marginRight: '3px', 				//in case the table contains any margins, colResizable needs to know the values used, e.g. "10%", "15em", "5px" ...
