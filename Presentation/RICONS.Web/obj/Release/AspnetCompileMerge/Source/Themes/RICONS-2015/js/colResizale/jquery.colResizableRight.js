(function($) {
    ...
    */
    var syncCols = function(t, i, isOver) {
        ...
        //table scrollTop
        $('#fixHeaderRight').find('th').eq(c.eq(0).index()).width(w + PX);
        $('#fixHeaderRight').find('th').eq(c2.eq(0).index()).width(w2 + PX);
        if (isOver) {
            c.w = w; c2.w = w2;
            $('#fixHeaderRight').find('th').eq(c.eq(0).index()).width(w);
            $('#fixHeaderRight').find('th').eq(c2.eq(0).index()).width(w2);
        }
        $('#fixHeaderLeft .header-box').height($('#fixBodyRight .header-box').innerHeight());
        $('#fixBodyLeft .header-box').height($('#fixBodyRight .header-box').innerHeight());
    };
    ...
    */
    $.fn.extend({
        colResizableRight: function(options) {
            var defaults = {
