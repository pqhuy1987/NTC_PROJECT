...
$(document).ready(function (e) {
    ...
    $('#btnUpdate_Pay_employeeworker').on('click', function (e) {
        e.preventDefault();
        debugger;
        ...
        if (listChecked.length > 0) {
            $(listChecked).each(function (index, selectedItem) {
                ...
                DataJson += "'nhansuden_ct_pb_ngaytra':'" + nhansuden_ct_pb_ngaytra + "',";
                DataJson += "}";
            ...
            $.ajax({
                type: 'POST',
                url: '/PayStaff/Save_Pay_employee_worker',
                datatype: "JSON",
                ...
                success: function (response) {
                    GetData(true);
                    alert('Cập nhật hồ sơ chuyển trả ban thành công!');
                },
    ...
    $('#btnUpdate_phongNS').on('click', function (e) {
        e.preventDefault();
        debugger;
        var listChecked = $('.chkCheck:checked');
        var data = [];
        ...
        if (listChecked.length > 0) {
            ...
            $.ajax({
                type: 'POST',
                url: '/PayStaff/Save_Pay_employe_PhongNS_hoantat',
                datatype: "JSON",
                //data: { ids: DataJson }, DataJson1.toString()
                data: "DataJson=" + DataJson,
                cache: false,
...
function GetData(isHeader, stt ) {
    ...
    fncGetData_loadkhong_header(linkContent + 'PayStaff/SelectRows_Danhsachnhansuchuyentra', DataJson, isHeader);
}
