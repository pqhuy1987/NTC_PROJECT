(function($) {
    ...
    */
    var syncCols = function(t, i, isOver) {
        ...
        //table scrollTop
        $('#fixHeaderLeft').find('th').eq(c.eq(0).index()).width(w + PX);
        $('#fixHeaderLeft').find('th').eq(c2.eq(0).index()).width(w2 + PX);
        if (isOver) {
            c.w = w; c2.w = w2;
            $('#fixHeaderLeft').find('th').eq(c.eq(0).index()).width(w + PX);
            $('#fixHeaderLeft').find('th').eq(c2.eq(0).index()).width(w2 + PX);
        }
        $('#fixHeaderRight .header-box').height($('#fixHeaderLeft .header-box').innerHeight());
        $('#fixBodyRight .header-box').height($('#fixHeaderLeft .header-box').innerHeight());
        UpdateHeightRows();
    };
    ...
    */
    $.fn.extend({
        colResizableLeft: function(options) {
            var defaults = {
