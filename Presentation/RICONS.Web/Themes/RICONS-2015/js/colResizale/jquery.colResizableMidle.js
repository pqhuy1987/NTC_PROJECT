(function($) {
    ...
    */
    var syncCols = function(t, i, isOver) {
        ...
        //table scrollTop
        $('#hideColHeaderLeft').width(w + PX);
        $('#fixHeaderLeft').width(w + PX);
        $('#fixBodyLeft').width(w + PX);
        $('#hideColHeader').width((w2 - getScrollTopWidth("overRight")) + PX);
        $('#fixHeaderRight tr.header').remove();
        $('#fixHeaderRight').append($('#fixBodyRight tr.header').clone());
        if ($.browser.mozilla) {//fix firefox
            $('#hideColHeader').css('left', $('#fixBodyRight').position().left);
        }
        if (isOver) {
            c.w = w; c2.w = w2;
            $('#hideColHeaderLeft').width(w);
            $('#hideColHeader').width((w2 - getScrollTopWidth("overRight")));
        }
        UpdateGrips(t.cs);
        
    };

    function UpdateGrips(tcs) {
        //update jscrip left
        var t = $('#overLeft .JCLRgrips .JCLRgrip');
        var th = $('#fixBodyLeft').find('th');
        $('#overLeft .JCLRgrips').width($('#fixBodyLeft').width()); //the grip's container width is updated
        for (var i = 0; i < t.length; i++) {	//for each column
            $(t[i]).css({			//height and position of the grip is updated according to the table layout
            left: $(th[i]).offset().left - $('#fixBodyLeft').offset().left + $(th[i]).outerWidth() + tcs / 2 + PX
            });
        }
        $('#overLeft .JCLRgrips .JCLRLastGrip').css({			//height and position of the grip is updated according to the table layout
            left: ($('#fixBodyLeft').width() + 1) + PX
        });

       t = $('#overRight .JCLRgrips .JCLRgrip');
       th = $('#fixBodyRight').find('th');
        $('#overRight .JCLRgrips').width($('#fixBodyRight').width()); //the grip's container width is updated
        for (var i = 0; i < t.length; i++) {	//for each column
            $(t[i]).css({			//height and position of the grip is updated according to the table layout
            left: $(th[i]).offset().left - $('#fixBodyRight').offset().left + $(th[i]).outerWidth() + tcs / 2 + PX
            });
        }
        $('#overRight .JCLRgrips .JCLRLastGrip').css({			//height and position of the grip is updated according to the table layout
            left: ($('#fixBodyRight').width() + 1) + PX
        });
    };
    ...
    */
    $.fn.extend({
        colResizableMidle: function(options) {
            var defaults = {
                ...
                fixColumns: 3, 				//disables all the enhancements performed in a previously colResized table	
                //events:
